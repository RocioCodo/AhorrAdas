...
const pintarOperaciones = (arr) => {
  ...
  btnBorrar.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const borrado = operaciones.filter(
  ...
  btnEditar.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let editado = operaciones.filter(
        (operacion) => operacion.id === e.target.dataset.id
      );
      ...
      btnEditarOperacion.addEventListener("click", () => {
        const filtrar = operaciones.filter(
          (operacion) => operacion.id === editado[0].id
        );
        ...
        const nuevasOp = operaciones.map((operacion) =>
          operacion.id === editado[0].id ? operacionEditada : operacion
        );

        localStorage.setItem("operaciones", JSON.stringify(nuevasOp));
        const operacionesEditadas = JSON.parse(
...
const filtros = (e) => {
  ...
  if (porFecha !== new Date()) {
    operaciones = operaciones.filter(
      (operacion) => new Date(operacion.fecha) >= porFecha
    );
  }
...
const totalesBalance = (arr) => {
  const operGanancias = arr
    .filter((operacion) => operacion.tipo === "GANANCIA")
    .reduce((prev, current) => prev + Number(current.monto), 0);

  gananciasDiv.innerHTML = `<div  class="d-flex justify-content-start fs-6 textt">Ganancias</div>
    <div class="d-flex justify-content-end ganancias-monto fs-6 text">+$${operGanancias}</div>`;

  const operGastos = arr
    .filter((operacion) => operacion.tipo === "GASTO")
    .reduce((prev, current) => prev + Number(current.monto), 0);

  gastosDiv.innerHTML = `<div  class="d-flex justify-content-start fs-6 text">Gastos</div>
    <div class="d-flex justify-content-end gastos-monto fs-6 text">-$${operGastos}</div>`;

  const totalesOp = operGanancias - operGastos;

  totalDiv.innerHTML = `<div  class="card-subtitle mb-2 text-muted fs-4 text">Total</div>
    <div class="d-flex justify-content-end fs-4 text">${
      totalesOp > "0" ? "+" : "-"
    }$${Math.abs(totalesOp)}</div>`;
};
...
btnAgregarCategoria.addEventListener("click", () => {
  ...
  listaCategorias.innerHTML += `<div class="categorias-div">
    <div>
...
const cargarSelects = () => {
  ...
  btnBorrarCat.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const obtenerCategoria = categorias.find(
        (categoria) => categoria.id === e.target.dataset.id
      ).nombre;
      let categoriaBorrada = categorias.filter(
        (categoria) => categoria.id !== e.target.dataset.id
      );
  ...
  btnEditarCat.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let categoriaEditada = categorias.filter(
        (categoria) => categoria.id === e.target.dataset.id
      );
      ...
      confirmarCategoriaBtn.addEventListener("click", () => {
        const filtrarCat = categorias.filter(
          (categoria) => categoria.id === categoriaEditada[0].id
        );
        ...
        const nuevasCategorias = categorias.map((categoria) =>
          categoria.id === categoriaEditada[0].id ? categoriaAEditar : categoria
        );
...
const categoriaMayorGanancia = (operaciones) => {
  ...
  const porCategoriaGananciaResumen = operaciones
    .filter((operacion) => operacion.tipo === "GANANCIA")
...
const categoriaMayorGasto = (operaciones) => {
  ...
  const porCategoriaGastoResumen = operaciones
    .filter((operacion) => operacion.tipo === "GASTO")
    .sort((a, b) => Number(b.monto) - Number(a.monto));
...
const totalPorCategoria = (operaciones, categorias) => {
  ...
  const conBalance = categorias.map((categoria) => {
    ...
    reportesCategorias.innerHTML += `<div class="row pt-3">
...
const totalPorMes = (arr) => {
  ...
  for (let i = 0; i < mesesSinRepetir.length; i++) {
    ...
    reportesMes.innerHTML += `<div class="row pt-3">
